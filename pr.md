...
- 어떻게하면 병렬적으로 테스트할 때 독립적으로 실행시킬까?
  ...
  1. test마다 독립적으로 events 리스트를 바라보아야한다.
  2. test마다 server handler에서 다르게 동작해야한다.

  handler에서 다르게 동작하려면 test 동작할 때 override를 해주고, 끝나면 복구시켜줘야한다.
...
테스트코드는 처음 공부해봐서 쉽지 않았다.
당당하게 hard를 도전했지만, 테스트작성에 대한 사고력부터 조금 잘못생각하기도 하고 어려웠다.
